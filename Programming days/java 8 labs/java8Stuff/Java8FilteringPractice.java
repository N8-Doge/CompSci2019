...
public class Java8FilteringPractice {
    ...
    public static List<String> playWithStringLengths(List<String> words, int avoidLength) {
        words.removeIf(n -> (n.length()==avoidLength+1 || n.length()==avoidLength-1 || n.length()==avoidLength));
        return words;
    }
    ...
    playWithStrings(["wi", "bandwid", "zeni"], "ith") returns ["bandwid"]
     */
    public static List<String> playWithStrings(List<String> words, String it) {
        words.replaceAll(n -> n+"th");
        words.removeIf(n -> n.contains(it));
        return words;
    }
}